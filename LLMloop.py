        ...
        for response_id, (code_run, rl_run) in enumerate(zip(code_runs, rl_runs)):
            ...
            if traceback_msg == '':
                ...
                if "gt_reward" in tensorboard_logs and "gpt_reward" in tensorboard_logs:
                    ...
                    if len(gt_reward) == 0 or len(gpt_reward) == 0:
                        logging.warning("gt_reward 或 gpt_reward 为空，跳过相关系数计算")
                        reward_correlation = float("nan")        # 或 0.0
                    else:
                        min_len = min(len(gt_reward), len(gpt_reward))
                        reward_correlation = np.corrcoef(
                            gt_reward[:min_len], gpt_reward[:min_len]
                        )[0, 1]
                ...
                for metric in tensorboard_logs:
                    if "/" not in metric:
                        ...
                        values = tensorboard_logs.get(metric, [])
                        if not values:                      # 空列表 / Dict 里没有这个键
                            logging.warning(f"No data for metric '{metric}' – skipping this metric this round.")
                            metric_cur_max = float("-inf")  # 或者 0.0/np.nan，看你想怎么记
                        else:
